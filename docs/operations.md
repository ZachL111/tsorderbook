...
`tsorderbook` is centered on trading systems work in TypeScript. The technical target is: Maintain price-time priority order books from event logs.
...
The score is deterministic. The current threshold is 152. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 5.
